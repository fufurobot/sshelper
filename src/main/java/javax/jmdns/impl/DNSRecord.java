...
public abstract class DNSRecord extends DNSEntry {
	...
	public static abstract class Address extends DNSRecord {
		...
		protected Address(String name, DNSRecordType type, DNSRecordClass recordClass, boolean unique, int ttl,
				InetAddress addr) {
		...
		boolean handleQuery(JmDNSImpl dns, long expirationTime) {
			if (dns.getLocalHost().conflictWithRecord(this)) {
				DNSRecord.Address localAddress = dns.getLocalHost().getDNSAddressRecord(this.getRecordType(),
						this.isUnique(), DNSConstants.DNS_TTL);
				if (localAddress != null) {
					int comparison = this.compareTo(localAddress);

					if (comparison == 0) {
						// the 2 records are identical this probably means we
						// are seeing our own record.
						// With multiple interfaces on a single computer it is
						// possible to see our
						// own records come in on different interfaces than the
						// ones they were sent on.
						// see section "10. Conflict Resolution" of mdns draft
						// spec.
		 ...
		 * (non-Javadoc)
		 * 
		 * @see javax.jmdns.impl.DNSRecord#getServiceEvent(javax.jmdns.impl.
		 * JmDNSImpl)
		 */
		@Override
		...
		protected void toString(StringBuilder aLog) {
			super.toString(aLog);
			aLog.append(
					" address: '" + (this.getAddress() != null ? this.getAddress().getHostAddress() : "null") + "'");
		}
	...
	public static class Text extends DNSRecord {
		...
		protected void toString(StringBuilder aLog) {
			super.toString(aLog);
			aLog.append(
					" text: '" + ((_text.length > 20) ? new String(_text, 0, 17) + "..." : new String(_text)) + "'");
		}
	...
	public static class Service extends DNSRecord {
		...
		public Service(String name, DNSRecordClass recordClass, boolean unique, int ttl, int priority, int weight,
		...
		boolean handleQuery(JmDNSImpl dns, long expirationTime) {
			...
			if (info != null && (info.isAnnouncing() || info.isAnnounced())
					&& (_port != info.getPort() || !_server.equalsIgnoreCase(dns.getLocalHost().getName()))) {
				logger1.finer("handleQuery() Conflicting probe detected from: " + getRecordSource());
				DNSRecord.Service localService = new DNSRecord.Service(info.getQualifiedName(), DNSRecordClass.CLASS_IN,
						DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL, info.getPriority(), info.getWeight(),
						info.getPort(), dns.getLocalHost().getName());
				...
				if (info.isProbing() && comparison > 0) {
					...
					info.setName(NameRegister.Factory.getRegistry().incrementName(dns.getLocalHost().getInetAddress(),
							info.getName(), NameRegister.NameType.SERVICE));
		...
		@Override
		public ServiceInfo getServiceInfo(boolean persistent) {
			return new ServiceInfoImpl(this.getQualifiedNameMap(), _port, _weight, _priority, persistent,
					(byte[]) null);
		}
	...
	public static class HostInformation extends DNSRecord {
		 ...
		 * @param os
		 */
		public HostInformation(String name, DNSRecordClass recordClass, boolean unique, int ttl, String cpu,
				String os) {
